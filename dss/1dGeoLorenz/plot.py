...
def wrapped_far(prm, nseg, W, n_repeat): 
    ...
    obt_, vb_ = zip(*results)
    phiavg_ = [obt.phiavg for obt in obt_]
    sc_ = [vb.sc_mean for vb in vb_]
    uc_ = [vb.uc_mean for vb in vb_]
...
def trajectory():
    ...
    plt.ylabel('$\\rho (x)$')
    plt.tight_layout()
    plt.savefig('x_frequency.png'.format(ds.B, ds.prm[1]))
    plt.close()
...
def all_info():
    ...
    starttime = time.time()
    # phiavg, sc, uc, x, nu, sc_, nut, LE, [div1, div2] = far(200, W)
    obt, vb = far(200, W)
    endtime = time.time()
    print('time for far:', endtime-starttime)
    print('phiavg, sc, uc, grad = ', '{}, {}, {}, {}'.format(obt.phiavg, vb.sc_mean, vb.uc_mean, vb.sc_mean-vb.uc_mean))
    ...
    ax1.scatter(obt.x[:,:], vb.nu[:,:])
    ax1.set_title('x vs nu')
    ax2.scatter(obt.x[:,:], vb.nut[:,:])
    ax2.set_title('x vs nut')
    # set_trace()
    # ax3.scatter(x[:,:,0], div1[:,:,0])
    # ax3.set_title('x[0] vs div1[0]')
...
def change_prm():
    ...
    prms = np.tile(np.linspace(-0.3, 0., NN), (nprm,1)).T
    phiavgs = nanarray(NN)
